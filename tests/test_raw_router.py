...
class TestFrames:
    ...
    async def test_golden_frame_per_zone(self, _fast_client, instance, on):
        ...
        assert await raw_router.async_zone_power(entity, on=on) is True
    ...
    @pytest.mark.parametrize("instance,zone_key", sorted(raw_router.ZONE_KEY_BY_TOGGLE.items()))
...
class TestGates:
    ...
    def test_lan_only_writes_ignore_the_suppression_window(self):
        ...
        target = lan_raw.lan_target(coordinator, DEVICE_ID, "H60B0")
    ...
    async def test_raw_frames_resume_when_the_suppression_clears(self, _fast_client):
        ...
        assert await raw_router.async_zone_power(entity, on=True) is True
...
class TestDelivery:
    ...
    async def test_frame_is_repeated(self, _fast_client):
        ...
        await raw_router.async_zone_power(entity, on=True)

        assert len(_fast_client.sends) == lan_raw.LAN_WRITE_REPEATS
        assert lan_raw.LAN_WRITE_REPEATS in (2, 3)
...
class TestOptimisticState:
    ...
    def test_constraint_is_read_from_the_profile(self):
        ...
        assert zone_state.displaced_zone_keys(profile, "downlight", lit) == ["ripple"]
        assert zone_state.displaced_zone_keys(unconstrained, "downlight", lit) == []
